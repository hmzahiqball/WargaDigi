...
<div class="card border-0 shadow-sm mb-4" style="border-radius: 16px;">
    <div class="card-body p-4 bg-white" style="border-radius: 16px;">
        ...
        <div class="d-flex gap-2 flex-wrap">
            <button class="btn btn-outline-primary btn-sm rounded-pill px-3" data-bs-toggle="modal" data-bs-target="#modalDetailRw{{ $item->id }}"><i class="bi bi-eye me-1"></i>Detail Permintaan</button>
    ...
    <div class="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable">
        <div class="modal-content border-0 shadow bg-white" style="border-radius: 16px;">
            ...
            <div class="modal-body px-4 pt-3 pb-4">
                    ...
                    <div class="col-md-7">
                        <div class="border rounded-3 p-3 h-100" style="border-color:#198754 !important;">
                            <div class="d-flex justify-content-between align-items-center mb-2">
                                ...
                                <span class="badge rounded-pill px-3 py-1" style="background:#D1ECF1;color:#0C5460;">Disetujui RT</span>
                            </div>
                ...
                <div class="mb-3">
                    <div class="d-flex justify-content-between align-items-center mb-2">
                        <span class="text-muted small fw-bold text-uppercase" style="letter-spacing:1px;" id="rwLampiranLabel{{ $item->id }}">Lampiran Dokumen (KTP)</span>
                        <a href="#" class="text-success fw-bold small text-decoration-none" id="rwLampiranToggleBtn{{ $item->id }}" onclick="event.preventDefault(); toggleRwLampiran('{{ $item->id }}');">Lihat Kartu Keluarga &rarr;</a>
                    </div>
                    <div id="rwLampiranKtpBox{{ $item->id }}" class="border rounded-3 p-3 text-center" style="background:rgba(13,110,253,0.02); min-height:180px;">
                        @if($item->file_ktp_url)
                            <img src="{{ $item->file_ktp_url }}" alt="KTP" class="img-fluid rounded shadow-sm" style="max-height:160px; cursor:pointer;" onclick="showRwImg('{{ $item->file_ktp_url }}', 'Preview KTP')">
                            <div class="mt-2"><button type="button" class="btn btn-sm btn-outline-primary rounded-pill px-3" onclick="showRwImg('{{ $item->file_ktp_url }}', 'Preview KTP')"><i class="bi bi-eye me-1"></i>Lihat Dokumen</button></div>
                        @else
                    ...
                    <div id="rwLampiranKkBox{{ $item->id }}" class="border rounded-3 p-3 text-center d-none mt-2" style="background:rgba(13,110,253,0.02); min-height:180px;">
                        @if($item->file_kk_url)
                            <img src="{{ $item->file_kk_url }}" alt="KK" class="img-fluid rounded shadow-sm d-block mx-auto" style="max-height:160px; cursor:pointer;" onclick="showRwImg('{{ $item->file_kk_url }}', 'Preview KK')">
                            <div class="mt-2"><button type="button" class="btn btn-sm btn-outline-info rounded-pill px-3" onclick="showRwImg('{{ $item->file_kk_url }}', 'Preview KK')"><i class="bi bi-eye me-1"></i>Lihat Dokumen</button></div>
                        @else
                            <div class="py-4"><i class="bi bi-file-earmark-x text-muted fs-1 d-block mb-2"></i><span class="text-muted small">Belum diunggah</span></div>
                        @endif
                    </div>
            ...
            <div class="modal-footer border-0 px-4 pb-4 justify-content-center gap-2">
                <button type="button" class="btn btn-outline-danger rounded-pill px-4" data-bs-dismiss="modal" data-bs-toggle="modal" data-bs-target="#modalRejectRw{{ $item->id }}"><i class="bi bi-x-circle me-1"></i>Tolak & Beri Catatan</button>
                <button type="button" class="btn btn-success rounded-pill px-4 fw-bold" data-bs-dismiss="modal" data-bs-toggle="modal" data-bs-target="#modalTtdRw{{ $item->id }}"><i class="bi bi-check-circle me-1"></i>Sahkan Dokumen</button>
            </div>
        </div>
    </div>
</div>

{{-- Modal TTD RW with Live Preview --}}
                ...
                <div class="row g-4">
                    <div class="col-lg-5">
                        ...
                        <div class="mb-3">
                            ...
                            <div class="border border-2 rounded-3 p-3 text-center" style="border-style: dashed !important; cursor: pointer;" onclick="document.getElementById('stempelRw{{ $item->id }}').click()">
                                <input type="file" id="stempelRw{{ $item->id }}" class="d-none" accept=".png" onchange="previewStempel(this, 'stempelPreviewRw{{ $item->id }}')">
                                <div id="stempelPlaceholderRw{{ $item->id }}"><i class="bi bi-file-earmark-image text-success fs-2 d-block mb-1"></i><span class="small">Unggah Stempel (PNG, Max 2MB)</span></div>
                                <div id="stempelPreviewRw{{ $item->id }}" class="d-none"><img src="" class="img-fluid" style="max-height:70px;" alt="Stempel"><p class="text-success small mt-1 mb-0"><i class="bi bi-check-circle me-1"></i>Siap</p><button type="button" class="btn btn-sm btn-outline-danger rounded-pill mt-1" onclick="clearStempel('Rw', '{{ $item->id }}')"><i class="bi bi-trash me-1"></i>Hapus Stempel</button></div>
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-7">
...
function toggleRwLampiran(id) {
    var ktpBox = document.getElementById('rwLampiranKtpBox' + id);
    var kkBox = document.getElementById('rwLampiranKkBox' + id);
    var label = document.getElementById('rwLampiranLabel' + id);
    var btn = document.getElementById('rwLampiranToggleBtn' + id);
...
document.addEventListener('DOMContentLoaded', function() {
    document.querySelectorAll('[id^="modalTtdRw"]').forEach(modal => {
        ...
        canvas.addEventListener('pointermove', function(e) {
            ...
            lx = p.x; ly = p.y;
            // Real-time update to iframe preview (RW column)
            const iframe = document.getElementById('previewFrameRw' + id);
            if (iframe) injectImgToIframe(iframe, '.rw-col .ttd-box', canvas.toDataURL('image/png'), 'ttd-img');
        });
